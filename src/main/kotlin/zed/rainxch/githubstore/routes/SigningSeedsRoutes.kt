    ...
    get("/signing-seeds") {
        ...
        if (platform != "android") {
            call.respond(
                ...
                mapOf("error" to "platform query parameter required; android is the only supported value for now"),
            )
            return@get
        }
        ...
        val since = call.request.queryParameters["since"]?.let { raw ->
            raw.toLongOrNull() ?: run {
                call.respond(HttpStatusCode.BadRequest, mapOf("error" to "since must be epoch milliseconds"))
                return@get
            }
        }
        if (since != null && since < 0) {
            call.respond(HttpStatusCode.BadRequest, mapOf("error" to "since must be non-negative"))
            return@get
        }

        val cursor = call.request.queryParameters["cursor"]?.let { token ->
            SigningFingerprintRepository.PageCursor.decode(token) ?: run {
                call.respond(HttpStatusCode.BadRequest, mapOf("error" to "invalid_cursor"))
                return@get
            }
        }
