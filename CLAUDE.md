...
| `POST /events` | **Deprecated 2026-04-26 — telemetry was killed in the E6 audit.** Returns `410 Gone` with a JSON deprecation notice (`{error, message, deprecated_at, see}`) and `Cache-Control: public, max-age=86400` so old clients stop dribbling 400s into the origin. No body is read. The `Events` table and `SignalAggregationWorker` are still wired for historical aggregation but no new rows are ingested. |
